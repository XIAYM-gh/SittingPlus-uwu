...
public class SittingPlusInitializer implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        // Payload Handling
        ServerPlayNetworking.registerGlobalReceiver(HelloV1C2SPayload.ID, (payload, context) -> context.responseSender()
                .sendPacket(new HelloV1S2CPayload(Constants.PROTOCOL_VERSION)));
        ...
        // Sync to new players
        ServerPlayConnectionEvents.JOIN.register((_handler, sender, _server) -> {
            for (Map.Entry<UUID, String> entry : PLAYER_SIT_POSES.entrySet()) {
                sender.sendPacket(new PoseSyncV1S2CPayload(entry.getKey(), entry.getValue()));
            }
        });
